class BodyEntriesController < ApplicationController
  ...
  def show
    unless helpers.diary_user_permission(@body_entry.diary_id, current_user.id) >= DiariesHelper::PERMISSION_READONLY
      respond_to do |format|
        @body_entry.errors.add(:diary, 'is not shared with you.')
        format.html { render :index, status: :unprocessable_entity }
  ...
  def edit
    unless helpers.diary_user_permission(@body_entry.diary_id, current_user.id) >= DiariesHelper::PERMISSION_EDIT
      respond_to do |format|
        @body_entry.errors.add(:diary, 'is not shared with you.')
        format.html { render :index, status: :unprocessable_entity }
        format.json { render json: @body_entry.errors, status: :unprocessable_entity }
  ...
  def create
    ...
    if helpers.diary_user_permission(@body_entry.diary_id, current_user.id) < DiariesHelper::PERMISSION_EDIT
      @body_entry.errors.add(:diary, 'is not shared with you.')
      respond_to do |format|
  ...
  def destroy
    if helpers.diary_user_permission(@body_entry.diary_id, current_user.id) < DiariesHelper::PERMISSION_OWNERSHIP
      respond_to do |format|
        @body_entry.errors.add(:diary, 'is not shared with you.')
        format.html { render :edit, status: :unprocessable_entity }
        format.json { render json: @body_entry.errors, status: :unprocessable_entity }
